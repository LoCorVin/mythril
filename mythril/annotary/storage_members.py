...
def get_storage_mapping_for_types(contract, struct_map, m_type, slot_counter=0, bit_counter=0):
    ...
    if m_type.endswith("]"):
        slot_counter, bit_counter = new_slot(slot_counter, bit_counter) # Array always start at a new slot
    ...
    if m_type.startswith('mapping('):
        ...
        bit_counter, slot_counter = 0, slot_counter+1
        return m_info, slot_counter, bit_counter
    ...
    if m_type == "string" or m_type == 'bytes':
        slot_counter, _ = new_slot(slot_counter, bit_counter)
