...
from .sn_utils import get_si_from_state, get_containing_file, find_contract_idx_range, get_signature, \
    hash_for_function_signature, get_matching_state, get_matching_state_multisearch
...
def get_annotated_members(contract, code, start, content, annotation_length):
    ...
    if ";" in content:
        if not any([part.startswith("var=") for part in content.split(";")]):
            raise SyntaxError(
                "';' is used to separate the optional explicit variable definition from the function restrictions, so only use it in combination with 'var=...,...,...'")
        annotated_members = []
        ...
        for member in contract.storage_members:
            if start > member.src[0] - contract.contract_range[0] and start <= member.src[0] + member.src[1] - \
                contract.contract_range[0]:
                return [member]  # inside_member
            elif start > member.src[0] - contract.contract_range[0] and (
                not preceded_member or member.src[0] > preceded_member.src[0]):
        ...
        if not preceded_member or preceded_member.src[0] - contract.contract_range[0] + preceded_member.src[
            1] < preceding_nwl_idx:
            raise SyntaxError("Member Annotation has to be in the same line as parts of a member definition")

        sameline_members = []
        for member in contract.storage_members:
            if member.src[0] - contract.contract_range[0] > preceding_nwl_idx and member.src[0] - \
                contract.contract_range[0] < start:
...
def init_annotation(contract, code, head, kw, start, end, origin, config):
    if kw == "check" or kw == "always":
        ...
        return CheckAnnotation(code[start:(end + content_prefix)] + content + ")", get_pos_line_col(code[:start]),
                               origin, config=config)
    if kw == "never":
        content, content_prefix = get_annotation_content(code, start + len(head))
        return CheckAnnotation(code[start:(end + content_prefix)] + content + ")", get_pos_line_col(code[:start]),
                               origin, config=config, type=False)
    elif kw == "invariant":
        content, content_prefix = get_annotation_content(code, start + len(head))
...
def is_mapping_inside_range(mapping, start_pos, end_pos):
    """
    Checks if the code segment described by mapping is located inside the specified bounce.

    :param mapping: of the code that should be between positions
    :param start_pos:   start position of the outer code
    :param end_pos:     end position of the outer code
    :return:            'True' if the the mapping code is inside the positions
    """
...
def get_persistant_state(contract, state):
    matching_states = get_matching_state_multisearch(contract.states, [(is_not_ignored, None, False, "BACKWARD"), (
        is_persiting_instruction, None, True, "FORWARD")], state)
...
class Violation:
    ...
    def __init__(self, anchor_state, violating_state, persistant_state, contract, additional=None, length=None,
                 vio_description="", rew_based=True):
        ...
        self.trace.functions = [get_function_from_constraints(contract, anchor_state.mstate.constraints,
                                                              isinstance(anchor_state.current_transaction,
                                                                         ContractCreationTransaction))]

        self.rew_based = rew_based

        self.src_info, self.src_mapping = get_si_from_state(contract, self.anchor_state.instruction['address'],
                                                            self.anchor_state)
    ...
    def get_dictionary(self, annotation_contract, filename_map):
        ...
        if self.rew_based and trace_f.hash not in [imp_func.hash for imp_func in
                                                   annotation_contract.original_contract.implemented_functions]:

            # Redefine anchor state in the case we are in a rewritten function dummy
            self.anchor_state = self.get_matching_state(is_outside_contract,
                                                        (self.contract, find_contract_idx_range(annotation_contract)),
                                                        self.anchor_state)
            self.src_info, self.src_mapping = get_si_from_state(self.contract,
                                                                self.anchor_state.get_current_instruction()['address'],
                                                                self.anchor_state)
        ...
        loc = get_origin_pos_line_col(
            origin_file_code[:self.offset - length_prefix_rew + (len(self.code) if self.rew_based else 0)])
        ...
        return {"level": get_status_string(self.status), "lvl_description": self.get_lvl_description(),
                ...
                "row": loc[1], "note": self.note, "col": loc[2], "code": self.code,
                "length": 1 if self.rew_based else self.length,
                "pos": loc[0], "vio_description": self.vio_description,
                "transaction_depth": self.trace.lvl if self.trace else None,
                "chained_functions": [
                    {"name": func.name, "signature": func.signature, "isConstructor": func.isConstructor,
    ...
    def get_lvl_description(self):
        ...
        if lvl == Status.UNCHECKED:
            ...
            # Should not happen.
            return "Violation that can be disregarded looking at the single transaction it appears in."
        elif lvl == Status.HOLDS:
            return "Violation that can be disregarded looking at the context, the other transaction that can be " \
                   "executed to this contract."
        elif lvl == Status.VDEPTH:
            return "A combination of multiple transactions might allow to trigger this violation. Exploring a chain " \
                   "that does this was not possible due to the depth restriction to the analysis. So the " \
                   "violation may not be exploitable."
        elif lvl == Status.VCHAIN:
            return "A combination of multiple transactions allows to exploit this violation of the annotation. " \
                   "However at least two or more transactions are necessary."
...
class Annotation:
    ...
    def get_dictionary(self, filename_map):
        ...
        for vio in [violation.get_dictionary(self.annotation_contract, filename_map) for violation in self.violations if
                    violation.status in [Status.VSINGLE, Status.VCHAIN, Status.VDEPTH]]:
            if vio not in violations:
                violations.append(vio)

        adict = {"title": self.title, "level": get_status_string(self.status),
                 "lvl_description": self.get_lvl_description(),
                 "ano_description": self.get_annotation_description(), "pos": self.origin[0], "line": self.origin[1],
                 "col": self.origin[2],
                 "ano_string": self.annotation_str, "length": len(self.annotation_str),
                 "violations": violations}
        return adict

    def add_violations(self, violations, contract, additional=None, length=None, vio_description="", rew_based=True,
                       set_anchor_state=None):
        # Ads new violations to this annotation and sets the status, can be called multiple times
        for vio in violations:
            anchor_state, violating_state, persistant_states = get_violation_states(contract, vio)
            self.violations.extend(
                [Violation(anchor_state if not set_anchor_state else set_anchor_state, violating_state,
                           per_state, contract, additional, length, vio_description, rew_based) for per_state in
                 persistant_states
    ...
    def get_creation_ignore_list(self):
        return [(val0, val1, val2, val3, self) for val0, val1, val2, val3, val4 in
                list(zip(self.enter_inst, self.exit_inst, self.viol_inst, self.viol_rew_instrs, self.viol_rts)) if
                val4 == 'c']
    ...
    def get_lvl_description(self):
        lvl = self.status
        if lvl == Status.UNCHECKED:
            return "Status of the annotation was not checked jet, nothing can be said about possibly existing " \
                   "violations."
        elif lvl == Status.HSINGLE:
            return "No possible violation found, no operation in a single transaction can be turned into a violation " \
                   "by setting it up via multiple transactions."
        elif lvl == Status.HOLDS:
            return "The other transactions in the contract prevent a violation from being exploitable. Changing the" \
                   " code of a transaction might create a violation in an other transaction."
        elif lvl == Status.VDEPTH:
            return "There are no confirmed violations to this annotation, but a combination of multiple transactions" \
                   " might allow to trigger one. Exploring these chains was not possible due to the depth restriction" \
                   " to the analysis. So the violation may not be exploitable."
        elif lvl == Status.VCHAIN:
            return "At least one violation has been found were the combination of multiple transactions can lead to a" \
                   " violation of the annotation. However at least two or more transactions are necessary."
        elif lvl == Status.VSINGLE:
            return "At least one violation has been found that can be triggered by executing one single transaction."

    def trans_violations_check(self, sym_tran,
                               sym_con):  # Or should i get the predfiltered transaction or even builded chains here
        raise NotImplementedError("Abstract function of Annotation abstraction")

    def get_violations_description(
        self):  # Returns a tuple or list of tuples with description, assoviated line and string to highlight.
        raise NotImplementedError("Abstract function of Annotation abstraction")
...
class CheckAnnotation(Annotation):
    ...
    def get_annotation_description(self):
        return "This annotation checks whether or not the specified condition '" + self.content + "' can be " + (
            "false" if self.type else "true") + " at " \
               + "this point in the program. An assert is inserted and symbolic execution tries to find " + (
                   "falsifying" if self.type else "fulfilling") + " " \
               + "assigments. The presence of the assert statement does not influence the later execution."

    def rewrite_code(self, file_code, contract_code,
                     contract_range):
        # In the default case it returns '' empty string, to delete it before handing it over to the compiler
        assert_rew = expand_rew("", contract_code, (
            "assert(" + (self.content if self.type else "!(" + self.content + ")") + ");", self.loc[0]))
...
class InvariantAnnotation(Annotation):
    ...
    def __init__(self, contract, annotation_str, loc, origin_loc, assign_state_references, config):
        ...
        # For inherited transactions that have to be rewritten, an assert is inserted in the delegation function
        self.rew_function_asserts = []
    ...
    def rewrite_code(self, file_code, contract_code,
        ...
        for function in get_transaction_functions(self.original_contract):
            # Dont' build invariant assertions for functions that do not change storage
            if function.constant == True or function.visibility not in ["public",
                                                                        "external"]:
                continue
            for term_pos in function.terminating_pos:
                ...
                if 0 <= term_pos[0] < len(contract_code):
                    ...
                    if term_pos[1] > 0:
                        ...
                        if return_content:
                            ...
                            self.rewritings.append(expand_rew(function.name, contract_code, (
                                "var " + ret_var + "=" + return_content + ";", term_pos[0])))
                            self.rewritings.append(assertion_rew)
                            self.rewritings.append(
                                expand_rew(function.name, contract_code, ("return " + ret_var + "; /*", term_pos[0])))
                    ...
                    if " returns " in function.head:
                        ...
                        rt_str = rt_str[
                                 find_first_uncommented_str(rt_str, "(") + 1:find_first_uncommented_str(rt_str, ")")]
                    ...
                    if return_types and len(return_types) > 0:
                        self.rewritings.append(expand_rew(function.name, contract_code, (
                            function.head + "{var " + ret_var + "=" + call_str, contract_end_pos)))
                    else:
                        self.rewritings.append(expand_rew(function.name, contract_code,
                                                          (function.head + "{" + call_str, contract_end_pos)))
                    self.rewritings.append(assertion_rew)
                    if return_types and len(return_types) > 0:
                        self.rewritings.append(
                            expand_rew(function.name, contract_code, ("return " + ret_var + ";}\n", contract_end_pos)))
                    else:
                        self.rewritings.append(expand_rew(function.name, contract_code, ("}\n", contract_end_pos)))
                    self.viol_rews.append(assertion_rew)
    ...
    def get_annotation_description(self):
        return "This annotation represents an invariant that should hold after every finished transaction " \
               "execution. The annotations condition '" + self.content + "' is checked by inserting an assert " \
                                                                         "statement at the exit points of every transaction function that can be executed by a transaction " \
                                                                         "and do change the contracts states."
...
def specific_code_location_with_mapping(contract, state):
    ...
    code = si.code.strip()
    return not code.startswith("function ") and not code.startswith(
        "contract ")  # and state.instruction["opcode"] in ['DELEGATECALL', 'CALLCODE']
...
def mapping_and_call_return(contract, state):
    ...
    si, mapping = si_and_mapping
    code = si.code.strip()
    return not code.startswith("function ") and not code.startswith("contract ") and state.instruction["opcode"] in [
        'DELEGATECALL', 'CALLCODE', "STATICCALL", "CALL"]
...
def is_persiting_instruction(_, state):
    return state.instruction['opcode'] in ['STOP', 'RETURN', 'SELFDESTRUCT'] and (
        not hasattr(state, "next") or not state.next)
...
class SetRestrictionAnnotation(Annotation):
    ...
    def __init__(self, contract, annotation_str, content, member_variables, loc, origin_loc, config):
        ...
        for m_var in member_variables:
            self.storage_slot_map[m_var.declaring_contract + "." + m_var.name] = contract.storage_map[
                m_var.declaring_contract + "." + m_var.name]
        self.member_variables = member_variables
        Annotation.__init__(self, annotation_str)

    def rewrite_code(self, file_code, contract_code, contract_range):
        # This annotation does not have to rewrite code
        return []
            ...
            for state in node.states:
                ...
                if state.instruction['opcode'] == "SSTORE" or unresolved_delegatecall:
                    ...
                    if not si_and_mapping or \
                        ...
                        delegate_states = get_matching_state(self.annotation_contract.states, mapping_and_call_return,
                                                             self.annotation_contract, state, False)
                        # When violation of same structor happens in different contract and thus not violating this annotated member var
                        if not delegate_states:
                            continue
                        anchor_state = get_anchor_state(self.annotation_contract, delegate_states[
                            0])  # Only one parent cause search goes upwards though tree
                    ...
                    if not in_constructor:
                        function = get_function_from_constraints(self.contract, anchor_state.mstate.constraints,
                                                                 in_constructor)
                        if function:
                    ...
                    delegate_res_signatures = [get_signature(rest_fun[len(SetRestrictionAnnotation.DELEGATE_PREFIX):])
                                               for rest_fun in self.restricted_f if
                                               rest_fun.startswith(SetRestrictionAnnotation.DELEGATE_PREFIX)]
                    ...
                    if function and (
                        function.name in self.restricted_f or function.signature in res_signatures) or in_constructor and (
                        "constructor" in self.restricted_f
                        or self.contract.name in self.restricted_f or any(
                        [restriction.startswith(self.contract.name + "(") for restriction in self.restricted_f])) \
                        or state_in_delegate and [hash for hash in
                                                  extract_possible_hashes(state, self.annotation_contract.name) if
                                                  hash in delegate_hashes] \
                        or state.environment.active_function_name in delegate_res_signatures:
                        continue

                        # Skipp if function is somehow mentioned in the restricted list
                    if mapfunction and (
                        mapfunction.name in self.restricted_f or mapfunction.signature in res_signatures):
                        continue
                    ...
                    if unresolved_delegatecall:
                        for member_name, storage_slots in self.storage_slot_map.items():
                            self.add_violations([state], self.annotation_contract, member_name,
                                                vio_description="This call statement could not be resolved and thus not analyzed. The call may write to the member variable '" +
                                                                storage_slots[0].member.name
                                                                + "' of type '" + storage_slots[
                                                                    0].member.type + "' although not allowed by the annotation: " + str(
                                                    self.annotation_str), rew_based=False,
                                                set_anchor_state=anchor_state)
                        ...
                        for member_name, storage_slots in self.storage_slot_map.items():
                            for storage_slot in storage_slots:
                                may_write_to, constraints = storage_slot.may_write_to(state.mstate.stack[-1],
                                                                                      ...
                                                                                      state.environment.active_account.storage._storage,
                                                                                      state.mstate.constraints,
                                                                                      consider_length=not
                                                                                      self.config.set_restricted[
                                                                                          'ignore_length_writing'])
                                if may_write_to:

                                    anchor_state = get_anchor_state(self.annotation_contract, anchor_state)
                                    si_and_mapping = get_si_from_state(self.annotation_contract,
                                                                       anchor_state.instruction['address'],
                                                                       anchor_state)

                                    src_info, mapping = si_and_mapping
                                    printd("Contract may write to forbidden slot: " + str(
                                        src_info.lineno) + ":: " + src_info.code)
                                    new_state = state
                                    ...
                                    self.add_violations([new_state], self.annotation_contract, member_name,
                                                        vio_description="This statement may write to the member variable '" + storage_slot.member.name
                                                                        + "' of type '" + storage_slot.member.type + "' although not allowed by the annotation: " + str(
                                                            self.annotation_str), rew_based=False,
                                                        set_anchor_state=anchor_state)
    ...
    def get_violations_description(self):
        # Returns a tuple or list of tuples with description, assoviated line and string to highlight.
        raise NotImplementedError("Abstract function of Annotation abstraction")

    def get_annotation_description(self):
        return "This annotation binds a restriction to one or more member variables in the contract and uses " \
               "symbolic execution to find executions were this restriction is violated. If the annotation is " \
               "inside of a single member variable declaration statement, the restriction is bound only to that " \
               "variable. If not it applies to all variables declared in the same line. \n This annotation " \
               "restricts setting values or content of the " \
               "variables: " + ", ".join([mv.type + " " + mv.name for mv in self.member_variables]) + " to the " \
                                                                                                      "specified function names or signatures: \n" + self.content
