...
class SymExecWrapper:
    ...
    def __init__(self, contract, address, strategy, dynloader=None, max_depth=22, execution_timeout=None, glbstate=None, instruction_filter=None):
        s_strategy = None
        if strategy == 'dfs':
        ...
        self.laser = svm.LaserEVM(self.accounts, dynamic_loader=dynloader, max_depth=max_depth, execution_timeout=execution_timeout, strategy=s_strategy, instruction_filter=instruction_filter)

        if not (glbstate is None):
            # Adding the ability to specify a custom global state, e.g. machine configuration from outside
            self.laser.sym_exec_glbstate(glbstate)
        else:
